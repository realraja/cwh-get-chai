  ...
  return (
    <nav className="min-h-[7vh] bg-gray-900 px-2 text-white flex justify-between items-center">
      <Link href={'/'} className="flex items-center justify-center gap-2 mx-2 cursor-pointer">
        <span><img className="h-10" src="/writing-cat.gif" alt="logo" /></span>
...
<button onClick={()=> setDropdown(!dropdown)} onBlur={()=> setTimeout(() => {
  setDropdown(false)
}, 250)} id="dropdownInformationButton" data-dropdown-toggle="dropdownInformation" className="text-white  focus:ring-4 focus:outline-none  font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center bg-purple-600 hover:bg-purple-700 focus:ring-purple-800" type="button">Welcome {session.user.email} <svg className="w-2.5 h-2.5 ms-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
<path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 4 4 4-4"/>
</svg>
</button>

{/* <!-- Dropdown menu --> */}
<div id="dropdownInformation"  className={`z-10 ${dropdown?'': 'hidden'} absolute top-12 left-20  divide-y  rounded-lg shadow w-44 bg-gray-700 divide-gray-600`}>
    ...
    <ul className="py-2 text-sm  text-gray-200" aria-labelledby="dropdownInformationButton">
      ...
      <li>
        <Link href="/profile" className="block px-4 py-2  hover:bg-gray-600 hover:text-white">Profile</Link>
      </li>
      <li>
        <Link href="#" className="block px-4 py-2  hover:bg-gray-600 hover:text-white">About Us</Link>
      </li>
    </ul>
    <div className="py-2">
